...
async function getOrCreateDailyDocs(dateKey) {
  ...
  if (pubSnap.exists && priSnap.exists) {
    return { publicRef, privateRef, publicData: pubSnap.data(), privateData: priSnap.data() };
  }

  const poolSnap = await db.collection('daily_word_pool').where('active', '==', true).get();
  if (poolSnap.empty) throw new Error('Pool diária vazia. Rode o seed primeiro.');

  // Seleção determinística por dia para evitar repetição acidental em retries.
  ...
  return { publicRef, privateRef, publicData: newPub.data(), privateData: newPri.data() };
}
...
exports.rotateDailyWord = onSchedule(
  ...
  async () => {
    ...
    logger.info('Palavra do Dia garantida', { dateKey, wordLength: publicData.wordLength });
  }
);

exports.startDailyRun = onCall({ region: 'southamerica-east1' }, async (req) => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Faça login para iniciar.');
...
exports.unlockDailyHint = onCall({ region: 'southamerica-east1' }, async (req) => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Faça login para desbloquear dica.');
  const uid = req.auth.uid;
  const dateKey = dateKeySaoPaulo(new Date());

  // TODO: validar prova de anúncio (ex: token do AdMob Rewarded SSV).
  const adProof = req.data?.adProof;
  if (!adProof) throw new HttpsError('failed-precondition', 'adProof obrigatório.');

  const runRef = db.doc(`daily_words/${dateKey}/runs/${uid}`);
  const runSnap = await runRef.get();
  if (!runSnap.exists) throw new HttpsError('failed-precondition', 'Inicie a Palavra do Dia antes.');

  const run = runSnap.data();
  if (run.completedAt) throw new HttpsError('failed-precondition', 'Partida já concluída.');

  const next = Math.min((run.unlockedHints || 3) + 1, 5);
  await runRef.update({ unlockedHints: next, updatedAt: admin.firestore.FieldValue.serverTimestamp() });

  const priSnap = await db.doc(`daily_words_private/${dateKey}`).get();
  if (!priSnap.exists) throw new HttpsError('not-found', 'Dados privados do dia ausentes.');
  const allHints = priSnap.data().hints || [];

  return { unlockedHints: next, hints: allHints.slice(0, next) };
});

exports.submitDailyGuess = onCall({ region: 'southamerica-east1' }, async (req) => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Faça login para validar tentativa.');
  const uid = req.auth.uid;
  const rawGuess = req.data?.guess;
  if (!rawGuess) throw new HttpsError('invalid-argument', 'guess obrigatório.');
  ...
  if (!runSnap.exists) throw new HttpsError('failed-precondition', 'Inicie a Palavra do Dia antes.');
  if (!priSnap.exists) throw new HttpsError('not-found', 'Resposta diária indisponível.');
  ...
  // Só retorna meaning após conclusão.
  ...
  return {
    ...
    share: {
      ...
      isRecord,
      dateKey,
      // Nunca incluir a palavra no payload de share.
      text: `MagicLexis Palavra do Dia ${dateKey} | Tentativas: ${attempts} | Tempo: ${Math.round(
        elapsedMs / 1000
      )}s${isRecord ? ' | Recorde!' : ''}`
    }
  };
